...
get_speed() {
    ...
    SPEED_DATA=$(curl -s https://api.fast.com/netflix/speedtest)
    
    # Extract download and upload speeds using grep and awk
    DOWNLOAD=$(echo "$SPEED_DATA" | grep -oP '"downloadSpeed":\K[0-9]+(\.[0-9]+)?')
    UPLOAD=$(echo "$SPEED_DATA" | grep -oP '"uploadSpeed":\K[0-9]+(\.[0-9]+)?')

    # Format the result
    echo "Download: ${DOWNLOAD} Mbps | Upload: ${UPLOAD} Mbps"
}
...
while true; do
	...
	echo -e "%{l}$workspaces_output" "%{c}" "%{r}  $(get_speed) $(Cal) $(Clock) $sound_output $battery_output"
	sleep 0.01s
