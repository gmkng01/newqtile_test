...
while true; do
    ...
    workspaces_output="%{F$workspaces_color}$(Workspaces)%{F-}"
    clock_output="%{F$clock_color}$(Clock)%{F-}"
    cal_output="%{F$cal_color}$(Cal)%{F-}"
    sound_output="%{F$sound_color}$(Sound)%{F-}"
    battery_output="%{T3}%{F$bk}%{F-}%{T-}%{B$bk}%{F$fr}$(get_battery_info)%{F-}%{B-}"

	echo -e "%{l}$(Workspaces)" "%{c}" "%{r} $(Clock) $(Cal) $(Sound) $battery_output"
	sleep 0.1s
done | lemonbar -p -B "#222831" -o -2 -f "NFS font-14" -o -2 -f "DejaVu Sans Mono for Powerline-12"
